...
namespace SCNetworking
{
    public class SCNetwork
    {
        public delegate void NetConnectionEventHandler<TEventArgs>(object sender, SCNetwork connection, TEventArgs e);
        public delegate void NetConnectionEventHandler(object sender, SCNetwork connection);
        ...
        private SCNetwork(TcpClient client)
        {
            this.client = client;
        }
        ...
        private void CallOnDataReceived(SCNetwork connection, byte[] data)
        {
            if (OnDataReceived != null)
                OnDataReceived(this, connection, data);
        }
        private void CallOnConnect(SCNetwork client)
        {
            if (OnConnect != null)
                OnConnect(this, client);
        }
        private void CallOnDisconnect(SCNetwork client)
        {
        ...
        private async Task ListenAsync()
        {
            while (true)
            {
                ...
                SCNetwork connection = new SCNetwork(client);
                StartReceiveFrom(connection);
                OnConnect(this, connection);
            }
        }

        private void StartReceiveFrom(SCNetwork client)
        {
            tasks.Add(ReceiveFromAsync(client));
        }
        private async Task ReceiveFromAsync(SCNetwork client)
        {
            try
            {
